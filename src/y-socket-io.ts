...
export interface Configuration {
     ...
     * @param doc The document instance
     */
    onDestroyDocument?: (doc: Document) => Promise<void> | void,
    /**
...
export class YSocketIO {
     ...
     */
    private _documents: Map<string, Document> = new Map<string, Document>();
    /**
     * @type {Server}
     */
    private io: Server;
     ...
     */
    private _levelPersistenceDir: string | undefined | null = null;
    /**
    ...
    constructor(io: Server, configuration?: Configuration) {
        ...
        if (!!this._levelPersistenceDir) this.initLevelDB(this._levelPersistenceDir);
    ...
    public initialize() {
        ...
        dynamicNamespace.use((socket, next) => {
            if (!this.configuration?.authenticate) return next();
    ...
    private async initDocument(name: string, namespace: Namespace, gc: boolean = true): Promise<Document> {
        const doc = this._documents.get(name) ?? (new Document(name, namespace, {
            ...
            onDestroy: (doc) => {
                ...
                if (!!this.configuration?.onDestroyDocument) this.configuration.onDestroyDocument(doc);
            },
        }));
        doc.gc = gc;
        if (!this._documents.has(name)) {
            if (!!this.persistence) await this.persistence.bindState(name, doc);
            this._documents.set(name, doc);
            if (!!this.configuration?.onLoadedDocument) {
                try {
     ...
     */
    private initLevelDB(levelPersistenceDir: string) {
     ...
     */
    private initSyncListeners = (socket: Socket, doc: Document) => {
     ...
     * @param {Document} doc The document
     */
    private initAwarenessListeners = (socket: Socket, doc: Document) => {
     ...
     * @param {Document} doc The document
     */
    private initSocketListeners = (socket: Socket, doc: Document) => {
        socket.on('disconnect', async () => {
            if ((await socket.nsp.allSockets()).size == 0) {
                if (!!this.configuration?.onAllConnectionsAreClosed) {
                    try {
     ...
     * @param {Document} doc The document
     */
    private startSynchronization = (socket: Socket, doc: Document) => {
