...
export class SocketIOProvider extends Observable<string> {
     ...
     * @private
     */
    private _broadcastChannel: string;
    /**
     ...
     * @private
     */
    private initAwarenessListeners = () => {
     ...
     */
    private onSocketConnection = (onConnect: ProviderConfiguration['onConnect'], resyncInterval: ProviderConfiguration['resyncInterval'] = -1) => {
     ...
     */
    private onSocketDisconnection = (event: Socket.DisconnectReason, onDisconnect: ProviderConfiguration['onDisconnect']) => {
     ...
     */
    private onSocketConnectionError = (error: Error, onConnectError: ProviderConfiguration['onConnectError']) => {
     ...
     */
    private onUpdateDoc = (update: Uint8Array, origin: SocketIOProvider) => {
        if (origin !== this) {
     ...
     */
    private onSocketSynUpdate = (update: ArrayBuffer) => {
        Y.applyUpdate(this.doc, new Uint8Array(update), this);
    }
     ...
     */
    private awarenessUpdate = ({ added, updated, removed }: AwarenessChange, origin: SocketIOProvider | null) => {
    ...
    private connectBc = () => {
        ...
        bc.publish(this._broadcastChannel, {
            type: 'awareness-update',
            data: AwarenessProtocol.encodeAwarenessUpdate(this.awareness, [this.doc.clientID]),
        }, this);
    };
     ...
     */
    private onBroadcastChannelMessage = (message: { type: string, data: any }, origin: SocketIOProvider) => {
        if (origin !== this && !!message && !!message.type) {
            switch (message.type) {
                case 'sync-step-1':
                    bc.publish(this._broadcastChannel, {
                        type: 'sync-step-2',
